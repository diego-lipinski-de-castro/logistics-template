...
class GenerateCitiesReport implements ShouldQueue //, ShouldBeUnique
{
    use Dispatchable;
    ...
    public function handle()
    {
        if ($this->range[0] == $this->range[1]) {
            $filename = 'Relatório de entregas das cidades exportado às ' . $this->now . '.xls';
        } else {
            $filename = 'Relatório de entregas das cidades entre ' . $this->range[0]->format('d-m-Y') . ' e ' . $this->range[1]->format('d-m-Y') . ' exportado às ' . $this->now . '.xls';
        }
        
        $sheets = new SheetCollection();

        $cities = City::enabled()->whereHas('deliveries')->get();

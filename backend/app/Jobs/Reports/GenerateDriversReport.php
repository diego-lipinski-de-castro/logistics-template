...
class GenerateDriversReport implements ShouldQueue //, ShouldBeUnique
{
    use Dispatchable;
    ...
    public function handle()
    {
        if ($this->range[0] == $this->range[1]) {
            $filename = 'Relatório de entregas dos entregadores exportado às ' . $this->now . '.xls';
        } else {
            $filename = 'Relatório de entregas dos entregadores entre ' . $this->range[0]->format('d-m-Y') . ' e ' . $this->range[1]->format('d-m-Y') . ' exportado às ' . $this->now . '.xls';
        }
        ...
        foreach ($drivers as $driver) {
            ...
            $sheets->put($driver->full_name ?? '-', $data);
        }

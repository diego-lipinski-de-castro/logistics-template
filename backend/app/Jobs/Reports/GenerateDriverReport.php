...
class GenerateDriverReport implements ShouldQueue
{
    use Dispatchable;
     ...
     * @return void
     */
    public function __construct(string $now, array $range = [], array $status = [], int $driverId)
    {
        $this->now = $now;
        $this->range = $range;
        $this->status = $status;
    ...
    public function handle()
    {
        ...
        if ($this->range[0] == $this->range[1]) {
            $filename = 'Relatório de entregas do entregador '. $this->driver->full_name .' exportado às ' . $this->now . '.xls';
        } else {
            $filename = 'Relatório de entregas do entregador '. $this->driver->full_name .' entre ' . $this->range[0]->format('d-m-Y') . ' e ' . $this->range[1]->format('d-m-Y') . ' exportado às ' . $this->now . '.xls';
        }

        $data = Delivery::query()
            ->with(['driver', 'user', 'user.city', 'steps'])
            ->ofDriver($this->driver->id)

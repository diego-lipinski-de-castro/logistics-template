  ...
  <developer-layout title="Mapa">
    ...
    <div class="py-12">
        ...
        <div class="grid gap-6 grid-cols-1 sm:grid-cols-2 md:grid-cols-4">
          <div v-for="(item, index) in stats" :key="index"
            ...
            <div class="flex items-center">
              <img :src="item.icon" class="rounded-md h-10 w-10" aria-hidden="true">
            ...
            <EyeIcon v-if="item.layerVisible" class="w-6 h-6 text-gray-500  hover:text-gray-900 cursor-pointer"
              @click="toggleLayer(item)" />
            <EyeOffIcon v-else class="w-6 h-6 text-gray-500  hover:text-gray-900 cursor-pointer"
              @click="toggleLayer(item)" />
          </div>
        </div>
      </div>
      ...
      <div class="grid gap-6 grid-cols-1 md:grid-cols-4 mt-5">
        <div class="col-span-3 relative overflow-hidden shadow-md rounded-md">
          <div id="map" class="z-40" style="height: 60vh" />
        ...
        <div class="col-span-1 hidden md:block">
          <div style="height: 60vh" class="flow-root bg-white  overflow-hidden shadow-md rounded-md">
            <nav class="flex flex-col md:flex-row relative z-0 divide-x divide-gray-200">
              <span
                ...
                <span class="absolute inset-x-0 bottom-0 h-0.5 bg-speedapp-orange-500" />
              </span>
            </nav>

            <div class="p-5">
              <input v-model="search" type="text" placeholder="Pesquisar" class="
                                    block
                                    w-full
            ...
            <ul role="list" style="height: calc(60vh - 134px + 1.25rem)"
              class="-mt-5 divide-y divide-gray-200 overflow-y-scroll px-5">
              <li v-for="(driver, index) in driversByName" :key="index" class="py-4">
                <div class="flex items-center space-x-4">
                  ...
                  <div>
                    <button v-if="driver.metadata.location !== null"
        ...
        <transition enter-active-class="transform ease-out duration-300 transition"
          ...
          <div v-if="status.message"
            ...
            <div class="p-4">
              <div class="flex items-center">
                <div class="shrink-0">
                  <CheckCircleIcon v-if="status.type == 'success'" class="h-6 w-6 text-green-400" aria-hidden="true" />

                  <XCircleIcon v-if="status.type == 'error'" class="h-6 w-6 text-red-400" aria-hidden="true" />
                </div>
...
// import scGeo from '@/assets/sc-geo.json';
import scGeo from '@/assets/sc2-geo.json';
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const driversByName = computed(() => {
      ...
      return props.drivers.filter((driver) => {
        return driver.full_name !== null && driver.full_name.toLowerCase().includes(search.value.toLowerCase())
      })
    });
    ...
    const updateDriverStatus = (event) => {
      ...
      Inertia.post(route("developer.drivers.updateStatus", driver), {
    ...
    const getDriverPopupContent = (driver) => {
      return `
                <div>
                    ...
                    <br>
                    <span>Versão: ${driver.metadata.version ?? '-'}</span>
                    <br>
